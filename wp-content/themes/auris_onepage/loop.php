...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
    <article id="post-<?php the_ID(); ?>" class="blogEntry">
		...
		if (wt_get_option('blog','meta_date')){
			...
			echo '<a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('d M').'</a></div>';
		}
		 ?>
			<?php //edit_post_link(__('<i class="blog_icon wt_icon-pencil"></i>', 'wt_front'),'',''); ?>
        ...
        <?php if ($featured_image_type == 'left') { echo '<div class="clearBoth"></div>'; } ?>        
        </div>
    </article>
