...
function wt_pagination($pages = '', $range = 2){
     ...
     if(1 != $pages)
     {
         ...
         for ($i=1; $i <= $pages; $i++)
         {
             ...
             {
                echo ($paged == $i)? "<span class='currentPosts'>".$i."</span>":"<a href='".get_pagenum_link($i)."' class=\"inactive\">".$i."</a>";			
             }
         }
...
function wt_nav_menu_remove_attributes( $menu ){
	return $menu = preg_replace('/ id=\"(.*)\"/iU', '', $menu );
}
add_filter( 'wp_nav_menu', 'wt_nav_menu_remove_attributes' );
/*
 * add a span element for style in the page
 */
function wt_comment_style($return) {
	return str_replace($return, "<i class=\"wt_icon-comments\"></i>$return", $return);
}
add_filter('get_comment_author_link', 'wt_comment_style');

function wt_span_before_link_list_categories( $list ) {
	$list = str_replace('<a href=','<i class="wt_icon-angle-right"></i><a href=',$list);
	return $list;
}
...
add_image_size( 'thumb', 70, 70, true);
add_image_size( 'portfThumb', 400, 242, true);
...
if(wt_get_option('general','scroll_to_top')) {
	add_filter('body_class','wt_add_wt_top');
	function wt_add_wt_top($classes) {
		$classes[] = 'wt-top';
		return $classes;
	}	
}

function wt_widget_text_shortcode($content) {
...
add_filter('widget_text', 'wt_widget_text_shortcode');
...
if(!function_exists('wt_add_home_link')) {
	if(wt_get_option('general','home_show')) {
		add_filter( 'wp_nav_menu_items', 'wt_add_home_link', 10, 2 );
		function wt_add_home_link($items, $args) {
			if(wt_get_option('general','menu_icons')) {
				$icons = '<i></i>';
				$icons_class = 'menu_icon ';
			}
			...
			if(is_single() || is_archive()){
				$homeMenuItem = '<li class="menu_icon_home menu-item menu-item-type-custom menu-item-object-custom icon_list level-1-li"><a href="' . get_site_url() . '/#wt_wrapper" class="'.$icons_class.'level-1-a">'. $icons .'<span data-hover="'.wt_get_option('general','home_text').'">'.wt_get_option('general','home_text').'</span></a></li>';
			}
			else {
				$homeMenuItem = '<li class="menu_icon_home menu-item menu-item-type-custom menu-item-object-custom icon_list level-1-li"><a href="#wt_wrapper" class="'.$icons_class.'level-1-a">'. $icons .'<span data-hover="'.wt_get_option('general','home_text').'">'.wt_get_option('general','home_text').'</span></a></li>';
			}
...
remove_action( 'wp_head', 'wt_adjacent_posts_rel_link_wp_head', 10, 0);
/*
...
function wt_media_player($type='full',$layout='',$height='',$width='') {
	...
	$media .= '<audio class="html5_audio" width="100%" height="30" controls="controls" preload="none" data-html5_audio_loop="false">
				   <source type="audio/mp3" src="'.$player_link.'" />
				
				   <object type="application/x-shockwave-flash" data="'.$uri.'/mediaelement/flashmediaelement.swf">
					   <param name="movie" value="'.$uri.'/mediaelement/flashmediaelement.swf" />
					   <param name="flashvars" value="controls=true&amp;file='.$player_link.'" />
					
				   </object>
			   </audio>';
		
	$html_output .= '<div class="audio_wrap"';
	if($type=='left'){
...
function wt_hex2rgb( $colour, $opacity ) {
        if ( $colour[0] == '#' ) {
...
function wt_get_slide($type='full',$layout='',$height='',$width='') {
	...
	$slide_link_1 = get_post_meta($post->ID,'_slide_image_1_url', true);
	$slide_link_2 = get_post_meta($post->ID,'_slide_image_2_url', true);
	$slide_link_3 = get_post_meta($post->ID,'_slide_image_3_url', true);
	$slide_link_4 = get_post_meta($post->ID,'_slide_image_4_url', true);
	$slide_link_5 = get_post_meta($post->ID,'_slide_image_5_url', true);
	$title_1 = get_post_meta($post->ID,'_slide_image_1_caption', true);
	$title_2 = get_post_meta($post->ID,'_slide_image_2_caption', true);
	$title_3 = get_post_meta($post->ID,'_slide_image_3_caption', true);
	$title_4 = get_post_meta($post->ID,'_slide_image_4_caption', true);
	$title_5 = get_post_meta($post->ID,'_slide_image_5_caption', true);
	...
	if ($slide_type == "flex") { 
		$li = '<li>';
		$end_li = '</li>';
		$flex_caption = '<div class="flex-caption">';
		$end_flex_caption = '</div>';
	} else {
		$li = '';
		$end_li = '';
		$flex_caption = '';
	...
	if (!empty ($slide_link_1)) { 
		$slide_type == "flex" && !empty($title_1) ? $output_flex_caption = $flex_caption . $title_1 . $end_flex_caption : $output_flex_caption = '';

		if($type=='left'){
			$content .= $li.'<img src="'. aq_resize( $slide_link_1, $img_width, $height, true ).'"';
		}else{
			$content .= $li.'<img src="'. aq_resize( $slide_link_1, $width, $height, true ).'"';
		}
		$content .= ' title="'.$title_1.'" />'.$output_flex_caption.$end_li;
	}
	if (!empty ($slide_link_2)) { 
		$slide_type == "flex" && !empty($title_2) ? $output_flex_caption = $flex_caption . $title_2 . $end_flex_caption : $output_flex_caption = '';
		if($type=='left'){
			$content .= $li.'<img src="'. aq_resize( $slide_link_2, $img_width, $height, true ).'"';
		}else{
			$content .= $li.'<img src="'. aq_resize( $slide_link_2, $width, $height, true ).'"';
		}
		$content .= ' title="'.$title_2.'" />'.$output_flex_caption.$end_li;
	}
	if (!empty ($slide_link_3)) { 
		$slide_type == "flex" && !empty($title_3) ? $output_flex_caption = $flex_caption . $title_3 . $end_flex_caption : $output_flex_caption = '';
		if($type=='left'){
			$content .= $li.'<img src="'. aq_resize( $slide_link_3, $img_width, $height, true ).'"';
		}else{
			$content .= $li.'<img src="'. aq_resize( $slide_link_3, $width, $height, true ).'"';
		}
		$content .= ' title="'.$title_3.'" />'.$output_flex_caption.$end_li;
	}
	if (!empty ($slide_link_4)) { 
		$slide_type == "flex" && !empty($title_4) ? $output_flex_caption = $flex_caption . $title_4 . $end_flex_caption : $output_flex_caption = '';
		if($type=='left'){
			$content .= $li.'<img src="'. aq_resize( $slide_link_4, $img_width, $height, true ).'"';
		}else{
			$content .= $li.'<img src="'. aq_resize( $slide_link_4, $width, $height, true ).'"';
		}
		$content .= ' title="'.$title_4.'" />'.$output_flex_caption.$end_li;
	}
	if (!empty ($slide_link_5)) { 
		$slide_type == "flex" && !empty($title_5) ? $output_flex_caption = $flex_caption . $title_5 . $end_flex_caption : $output_flex_caption = '';
		if($type=='left'){
			$content .= $li.'<img src="'. aq_resize( $slide_link_5, $img_width, $height, true ).'"';
		}else{
			$content .= $li.'<img src="'. aq_resize( $slide_link_5, $width, $height, true ).'"';
		}
		$content .= ' title="'.$title_5.'" />'.$output_flex_caption.$end_li;
	}
	if ($slide_type == "nivo") {
		$html_output .= '<div class="slide_wrap nivo_container"';
		if($type=='left'){
			$html_output .= ' style="width:'.$width.'px"';
		}
