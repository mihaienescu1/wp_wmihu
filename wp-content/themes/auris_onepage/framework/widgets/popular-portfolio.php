<?php
/**
 * popular_portfolio_Portfolio_Posts Widget Class
 *
 * @since 2.8.0
 */
class Wt_Widget_Popular_Portfolio_Posts extends WP_Widget {

	function Wt_Widget_Popular_Portfolio_Posts() {
		$widget_ops = array('classname' => 'widget_popular_portfolio_posts', 'description' => __( "The most popular portfolio posts", 'wt_admin') );
		$this->WP_Widget('popular_portfolio_posts', THEME_SLUG.' - '.__('Popular Portfolio Posts', 'wt_admin'), $widget_ops);
		$this->alt_option_name = 'widget_popular_portfolio_posts';
...
<?php elseif($display_extra_type == 'comments'):?>
					<span class="comments"><?php echo comments_popup_link(__('No response ','randf_front'), __('1 Comment','randf_front'), __('% Comments','randf_front'),''); ?></span>
...
<?php
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['theme_widget_popular_portfolio_posts']) )
			delete_option('theme_widget_popular_portfolio_posts');

		return $instance;
	}

	function flush_widget_cache() {
		wp_cache_delete('theme_widget_popular_portfolio_posts', 'widget');
	}
	...
	function form( $instance ) {
		...
		$categories = get_categories('orderby=name&hide_empty=0&taxonomy=wt_portfolio_category');
?>
