...
class Wt_Widget_Custom_Links extends WP_Widget {
	...
	function Wt_Widget_Custom_Links() {
		...
		$this->WP_Widget('custom_links', THEME_SLUG.' - '.__('Custom Links', 'wt_admin'), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		if( $count > 0){
			for($i=1; $i<= $count; $i++){
				...
				if(empty($image)){
					$image = THEME_IMAGES.'/customLink.png';
				}
				$output .= '<li class="'.$linktitle.'"><a href="'.$link.'" target="'.$target.'" title="'.$linktitle.'"><img src="'.$image.'" alt="'.$linktitle.'"/><span>'.$linktitle.'</span></a></li>';
			}
		}
