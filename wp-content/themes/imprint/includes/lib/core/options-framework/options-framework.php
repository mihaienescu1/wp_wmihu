...
function mudthemes_optionsframework_setdefaults() {
	...
	if ( isset($values) ) {
		//add_option( $option_name, $values ); // Add option with default settings :::::::: Disable Sane Defaults Requirement
	}
}
...
function mudthemes_optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		add_settings_error( 'imprint-options', 'restore_defaults', __( '<i class="fa fa-undo"></i> Options reset', 'imprint' ), 'updated fade' );
	 ...
	 */
		...
		add_settings_error( 'imprint-options', 'save_options', __( '<i class="fa fa-check"></i> Options saved', 'imprint' ), 'updated fade' );
		return $clean;
	}
...
?>                      <div class="mud-header-menu">
                            <a href="<?php echo IMPRINT_DOCS_URL ?>" target="_blank" title="Documentation"><i class="fa fa-edit"></i><span>Documentation</span></a>
                            <a href="<?php echo IMPRINT_SUPPORT_URL ?>" target="_blank" title="Support"><i class="fa fa-user"></i><span>Support</span></a>
                            <a href="<?php echo IMPRINT_CONTACT_URL ?>" target="_blank" title="Contact"><i class="fa fa-envelope"></i><span>Contact us</span></a>
                        </div>
                        <div class="mud-admin-logo">
                            <?php echo '<img src="' . IMPRINT_ADMIN_IMAGES . 'logo.png">'; ?>
                            <span class="mud-admin-version"><?php echo 'version ' .$imprint_version ?> | Settings <i class="fa fa-tas"></i></span>
                        </div>
    			<div class="upgrade-box"><p><?php if(!IMPRINT_PRO): _e('<i class="fa fa-quote-left"></i> Upgrade to Imprint Pro Version <i class="fa fa-quote-right"></i> and get 5 premium skins with 500+ Google Fonts, full-width layout, extra color options and much more.', 'imprint'); ?> <a href="<?php echo IMPRINT_PRO_URL ?>" target="_blank" title="Download Imprint Pro"><span>Download Imprint Pro</span></a> <span class="span-header-download"> <i class="fa fa-check"></i></span></p><?php else: _e(sprintf('ImprintPro theme is active - we recommend that you must refer to documentation before getting started. Also see if any new update is available: <a href="%s" target="_blank">Check Here</a>', IMPRINT_CHECK_VERSION_URL ), 'imprint'); endif ?></div>
...
function of_mud_scroll_top(){
    ...
    $output = '';
    $output = '<div class="mud-scroll-div"><a title="Scroll Down "class="scroll" href="#optionsframework-submit" data-speed="2000" data-easing="easeInQuad" data-url="false"><i class="fa fa-download"></i></a></div>';
    echo $output;
