...
class Mudthemes_Options_Framework_Media_Uploader {
	...
	function optionsframework_media_scripts( $hook ) {
		...
		$menu = 'imprint-options';

		if ( 'appearance_page_' . $menu != $hook )
	        return;

		if ( function_exists( 'wp_enqueue_media' ) )
			wp_enqueue_media();
                
                //wp_register_script( 'of-medialibrary-uploader', IMPRINT_ASSETS .'admin/js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
		wp_register_script( 'of-media-uploader', IMPRINT_ASSETS .'admin/js/of-medialibrary-uploader.js', array( 'jquery' ) );
