...
function mudthemes_optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "heading") {
			$jquery_click_hook = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($value['name']) );
                        $fa_hook = $jquery_click_hook; //By mudthemes (just duplicate)
			$jquery_click_hook = "of-option-" . $jquery_click_hook;
                        $menu .= '<div class="mud-nav-tab">';
			$menu .= '<a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="mudthemes-nav" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">';
                        $menu .= '<i class="fa mud-fa-' . esc_attr( $fa_hook ) . '"></i>';                        
                        $menu .= '<span>' . esc_html( $value['name'] ) . '</span></a>';
                        $menu .= '</div>';
		}
	}
...
function mudthemes_optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != "divstart" ) && ( $value['type'] != "divend" ) ) {
			...
			if ( isset( $value['name'] ) ) {
				$output .= '<h4 class="heading">' . wp_kses( $value['name'], $imprint_kses_tags) . '</h4>' . "\n";
			}
		...
		case "checkbox":
			...
			$output .= '<label class="explain" for="' . esc_attr( $value['id'] ) . '">' . wp_kses( $explain_value, $imprint_kses_tags) . '</label>';
			break;
		...
		// Uploader
		case "upload":
                        $output .= Mudthemes_Options_Framework_Media_Uploader::optionsframework_uploader( $value['id'], $val, null );
		...
		case 'typography':
			...
			$typography_options = array(
				'sizes' => mudthemes_of_recognized_font_sizes(),
				'faces' => mudthemes_of_recognized_font_faces(),
				'styles' => mudthemes_of_recognized_font_styles(),
				'color' => true
			);
		...
		case 'background':
			...
			if (!isset($background['image'])) {
				$background['image'] = '';
                        ...
                        $output .= Mudthemes_Options_Framework_Media_Uploader::optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
			//$output .= optionsframework_medialibrary_uploader( $value['id'], $background['image'], null, '',0,'image');

/* Edited by mudthemes (Starts) */
...
/*			$class = 'of-background-properties';
			...
			$repeats = mudthemes_of_recognized_background_repeat();
			...
			$positions = mudthemes_of_recognized_background_position();
			...
			$attachments = mudthemes_of_recognized_background_attachment();
...
/* Edited by mudthemes (Ends) */

			break;

		// Editor
		case 'editor':
			$output .= '<div class="explain">' . wp_kses( $explain_value, $imprint_kses_tags) . '</div>'."\n";
			echo $output;
		...
		case "info":
			...
			if ( isset($value['name']) ) {
				$output .= '<h2 class="heading">' . esc_html( $value['name'] ) . '</h2>' . "\n";
			}
			if ( $value['desc'] ) {
		...
		case "heading":
			...
			$output .= '<div class="mudthemes-resp-nav"><a href="#' . esc_attr( $jquery_click_hook ) . '">' . esc_html( $value['name'] ) . '</a></div>';
			$output .= '<div class="group" id="' . esc_attr( $jquery_click_hook ) . '">';
			$output .= "\n";
			break;
			
		case "gap":
//			$output .= '<div style="padding-bottom:10px;"></div>';
			break;
		
		case "divstart":
			$id = 'id="' . esc_attr( $value['id'] ) . '" ';
			$output .= '<div class="subsection" ' . $id . '>';
			break;
		...
		case "divend":
			...
			break;
                ...
                case "menu_upgrade";
                        ...
                        $output .= '<span><i class="fa fa-check-square-o"></i> Menu Styler is only available in Imprint Pro Version. <a href="'. IMPRINT_PRO_URL .'" target="_blank">Upgrade now</a> to get more menu styling options.</span>';
                        $output .= '</div>';
                        break;
                        
                case "font_upgrade";
                        $output .= '<div class="mud-options-upgrade-box font-upgrade-box">';
                        $output .= '<span><i class="fa fa-check-square-o"></i> Get access to 500+ Google Fonts. <a href="'. IMPRINT_PRO_URL .'" target="_blank">Upgrade Now</a> to Imprint Pro version.</span>';
                        $output .= '</div>';
                        break;

                case "carousel_upgrade";
                        $output .= '<div class="mud-options-upgrade-box carousel-upgrade-box">';
                        $output .= '<span><i class="fa fa-check-square-o"></i> Use upto 15 different slides in a slideshow. <a href="'. IMPRINT_PRO_URL .'" target="_blank">Upgrade Now</a> to Imprint Pro version.</span>';
                        $output .= '</div>';
                        break;
